...
# The function evaluates the random derivatives of Px, determine which linear combination of the derivatives has highest Gowers norm  value and recursively call Factor.
	...
	# Recall, polynomial Px (= L*Z) is of degree equal to 'deg_of_sec_poly+1' in the algebraic form.
	# Variable l1 represents the list form of polynomial L.
	# Der will represent the collection of the random derivatives of Px.
	Der=[]


	# If deg_of_sec_poly==1 then generate the derivatives and check does linear polynomial l1 lie in the span of derivatives? 
	...
	# Suppose r denotes the list of random values of variables. In every iteration, we randomly choose a new r.
	...
	# tmp refer to the linear combination of the derivatives present in Der which has the highest gowers norm
	tmp=0
	for j in range(random_deri):
		tmp=tmp+si[j]*Der[j]

	# Calculate new Px (below named as POLY)
	Monom=tmp.monomials()
...
# The below function 'Exper' generates the polynomial P=L.Z where L and Z are randomly chosen polynomials of degrees 1 and deg_of_sec_poly repsectively and we try to regain back the L and Z.
