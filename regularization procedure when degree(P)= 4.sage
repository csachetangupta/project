# We will represent polynomials in the list form.
# Specifically, the list form is the list of the coefficients of the monomials in lexical order with decreasing degree.
# Ex. If NV=2=number of variables, then polynomial P of degree 3 is represented as list of coefficients of monomials in the order [x0*x0*x0, x0*x0*x1, x0*x1*x0, x0*x1*x1, x1*x0*x0, x1*x0*x1, x1*x1*x0, x1*x1*x1, x0*x0,x0*x1,x1*x0,x1*x1,x0,x1,constant].
...
#num is the decimal number to be converted into base-p representation
...
# To represent variable "hai", we will use its index(hai) which is =(a-1)*NV+i.
# Below we have defined the variables which we will use:-
# CO will hold the coefficients of the monomials of the IP.
...
# As instance for given above P, IP can also be represented by NEW and CO as defined below:-
...
#*******************PART 1******************
# Part1 determines the variables NEW and CO.
          ...
          for i in range(len(Poly)):
              if(Poly[i]!=0):
                  tmp=i;L=[]
# L will contain the indices of the variables of the corresponding monomial defined by its index.
# L=[i,j,k] for monomial=x[i]*x[j]*x[k] which was positioned at i*NV^2+j*NV+k in the list Poly.
...
# Random value of hi[j] will be at position (i-1)*NV+j in list r. So, random_val(hi[j]) = r[(i-1)*NV+j].
# tmp is the collection of the random derivatives of the DP.
# Variable 'constant' corresponds to the constant term of the random derivative of the DP.
# a,b,c correpond to the values of the variables of a particular monomial of DP in random list r. Example if monomial=h10*h21*h32, then A=0,B=NV+1,C=2*NV+2 and a,b,c are their corresponding values in the random list r.
          tmp=[]
          ...
          if(MaxDegree==2):
              for j in range(k):
                    ...
                    t=[0]*(MaxDegree*NV)
# t will be the list form of the random derivative without constant term.
...
# getPermutations function return sum of the product of the lists over all the possible permutations of the lists in L.
# For example if L=[L1,L2,L3] then getPermutations return flattened value of M=L1*L2*L3+L1*L3*L2+ L2*L1*L3+L2*L3*L1+ L3*L1*L2+L3*L2*L1.
         if(len(L)==1):
             return L[0]
...
# GN equals to the Gowers norm value of the polynomial POLY.
...
# IPValueAtL is the value of the IP with list L as its variables values. Note IPValueAtL equals to (POLY*transpose(SOL))[0][0] (=dotproduct(POLY,SOL)).
...
# Poly gets assigned to the list of the coefficients of all those monomials whose degree equal to MaxDegree (i.e. till NV^MaxDegree).
                      for i in range(LenOfFact):
                          if(c[i]!=0):
...
# PL is the collection of poly's for which linear independent set has to be found.
...
# This function represents the single step to uniform the 'factor'. The single step comprises of checking the "Epsilon" uniformity of the "factor". If it is already uniform, then return. Else estimate the polynomial Poly by random derivatives say D_1,....D_k obtained from function BVLemma and again make uniform the factor defined by D_1,...D_k, it seems we are going in circles. It proceeds by lifting back the obtained uniform factor of the factor D_1,...D_k in terms of original variables.
...
# List represents the uniform factor of the low degree factor defined by the derivatives D_1,....D_k (and D_1,....D_k are defined more properly in the function UniformRefineOneStep).
# V holds the collection of regular factors till c becomes a zero tuple. Once c will become [0,...,0], we will append V to IPoly.
        V=[]
        while True:
...
# IPolyList has the input degree 4 polynomial in the list form.
...
# DL is the degree of polynomials in PL.
...
SampleSize=p^2+75                                             # "SampleSize"=number of samples to be used for calculating Gowers norm
...
# Here, M represents the uniform factor in list form and Quad is the polynomial in algebraic form.
        vec = [0]*(NV^2+NV+1)
...
# This function searches for Epsilon among the values defined by EpsilonE, EpsilonS and diff such that pol2 and pol4 can be written as a linear combination of the polynomials in the uniform factor.
...
# This function finds the number of bad iterations where bad means that particular generator polynomial (eg. pol1, pol2) cannot be written as a linear combination of the polynomials in the uniform factor.
# newk specifies the value of k(=#of derivatives to be evaluated in Bogdanov Viola Lemma)
...
# This function generates the list of lengths of the uniform factor for Epsilon's.
# newk specifies the value of k(=#of derivatives to be evaluated in Bogdanov Viola Lemma)
# EpsilonS and EpsilonE specifies the starting and ending values of Epsilon
# diff denotes the difference between two consecutive values of Epsilon
