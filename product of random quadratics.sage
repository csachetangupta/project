...
lowranklc=0	
# lowranklc refer to the linear combination of the derivatives present in derprod which has the highest gowers norm
for j in range(3):
...
Der=[]
tmp=lowranklc
# verify that linear polynomials does lie in the span of the derivative of tmp or not?
for i in range(17):
# we have evaluated 17 derivatives in random directons
		...
		Der.append(t)
# arr is the matrix constructed corresponding to the derivatives in Der
arr=matrix(GF(p),Der)
for j in range(3):
		try:
...
# WE SHOWED ABOVE THAT ALL OF THESE SINGLE DERIVATIVES can be recovered. Then, it is easy to recover R1,R2 and R3 since we explicitly know h1,...,h6.
