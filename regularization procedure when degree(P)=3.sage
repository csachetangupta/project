...
# To represent variable "hai", we will use its index(hai) which is defined as (a-1)*NV+i.
# Below we have defined the variables which we will use:-
# CO will hold the coefficients of the monomials of the DP.
...
# As instance for given above Poly, DP can also be represented by Ind and CO as defined below:-
...
#*******************PART 1******************
# Part1 determines the variables Ind and CO.
...
# DL will represent the list of the degrees of the polynomials in the PL.
...
# DPValueAtL is the value of the DP with list L as its variables values. Note DPValueAtL equals to (POLY*transpose(SOL))[0][0] (=dotproduct(POLY,SOL)).
...
# This function searches for Epsilon among the values defined by EpsilonS, EpsilonE and diff such that pol2 and pol4 can be written as a linear combination of the polynomials in the uniform factor.
...
# This function finds the number of bad iterations where bad iteration means that the generator polynomial (eg. pol1, pol2) cannot be written as a linear combination of the polynomials in the uniform factor.
# newk specifies the value of k(= # of derivatives to be evaluated in Bogdanov Viola Lemma).
...
# This function generates the list of the lengths of the uniform factor for all Epsilon's.
# newk specifies the value of k(=#of derivatives to be evaluated in Bogdanov Viola Lemma).
# EpsilonS and EpsilonE specifies the starting and ending values of Epsilon.
# diff denotes the difference between two consecutive values of Epsilon.
